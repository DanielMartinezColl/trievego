...
@Component({
    selector: 'jhi-eventousuario',
    templateUrl: './eventousuario.component.html'
})
...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.eventousuarioService.search({
                page: this.page - 1,
        ...
        this.eventousuarioService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<Evento[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/eventousuario'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/eventousuario', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    registerChangeInEventos() {
        this.eventSubscriber = this.eventManager.subscribe('eventousuarioListModification', (response) => this.loadAll());
    }
