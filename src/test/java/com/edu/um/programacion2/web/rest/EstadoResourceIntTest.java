...
public class EstadoResourceIntTest {
    ...
    public void setup() {
        ...
        final EstadoResource estadoResource = new EstadoResource(estadoRepository, estadoSearchRepository);
    ...
    public void createEstado() throws Exception {
        ...
        // Validate the Estado in Elasticsearch
        Estado estadoEs = estadoSearchRepository.findOne(testEstado.getId());
        assertThat(estadoEs).isEqualToIgnoringGivenFields(testEstado);
    }
    ...
    public void deleteEstado() throws Exception {
        ...
        boolean estadoExistsInEs = estadoSearchRepository.exists(estado.getId());
        assertThat(estadoExistsInEs).isFalse();
    ...
    @Test
    @Transactional
    public void searchEstado() throws Exception {
        // Initialize the database
        estadoRepository.saveAndFlush(estado);
        estadoSearchRepository.save(estado);

        // Search the estado
        restEstadoMockMvc.perform(get("/api/_search/estados?query=id:" + estado.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(estado.getId().intValue())))
            .andExpect(jsonPath("$.[*].nombre").value(hasItem(DEFAULT_NOMBRE.toString())));
